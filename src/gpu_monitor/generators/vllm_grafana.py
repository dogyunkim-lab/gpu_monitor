...
def _build_panels() -> list:  # noqa: C901 — 패널 구성 함수라 길이 허용
    ...
    # ================================================================
    # Section 7: CPU & 프로세스 모니터링
    # ================================================================
    # node_exporter (VM 전체 CPU) + vLLM process metrics (프로세스별 CPU)
    nf = '{instance=~"$instance"}'  # node_exporter는 model_name 없음

    panels.append(_row(pid, "CPU & 프로세스 모니터링", y)); pid += 1; y += 1

    # VM별 CPU 사용률 (node_exporter)
    panels.append(_ts_panel(pid, "VM CPU 사용률 (%)",
        ...
        "· E2E gap이 큰 시점과 CPU 피크가 겹치면 CPU 병목 확정",
        [_target(
            '100 - (avg by (vm) (rate(node_cpu_seconds_total{mode="idle"}[1m])) * 100)',
            "{{vm}}", "A",
        )],
        {"h": 8, "w": 8, "x": 0, "y": y},
    ...
    panels.append(_ts_panel(pid, "Load Average (1분) vs CPU 코어",
        "1분 평균 부하와 CPU 코어 수 비교.\n"
        "Load > 코어 수 → 프로세스가 CPU를 기다리는 상태.\n\n"
        "vLLM TP=2 기준 최소 4 코어 필요:\n"
        "  1 API server + 1 Engine Core + 2 GPU Worker\n"
        "코어 부족 시 Engine Core(busy loop)가 CPU를 독점하여\n"
        "API server가 starve → 토크나이저/스트리밍 지연",
        [
            _target("node_load1", "Load 1분 평균", "A"),
            _target('count(count by (cpu) (node_cpu_seconds_total{mode="idle"}))', "CPU 코어 수", "B"),
        ],
        {"h": 8, "w": 8, "x": 8, "y": y},
        unit="none",
    )); pid += 1

    # vLLM 프로세스 CPU 사용률 (vLLM 자체 /metrics에서 노출)
    panels.append(_ts_panel(pid, "vLLM 프로세스 CPU 사용률",
        "vLLM API server 프로세스의 CPU 사용 시간 변화율.\n"
        "(prometheus_client가 자동 노출하는 process_cpu_seconds_total)\n\n"
        "판정 기준:\n"
        "· 값이 1.0 = CPU 1코어를 100% 점유\n"
        "· E2E gap이 큰 시점에 이 값이 높으면\n"
